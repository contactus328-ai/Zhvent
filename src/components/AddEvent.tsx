...
export function AddEvent({ navigate, addEvent, editingEvent, isNewFromDashboard }: AddEventProps) {
  ...
  const eventTypes = ['Dancing', 'Singing', 'Acting', 'Treasure Hunt', 'Debate'];
  ...
  useEffect(() => {
    if (editingEvent) {
      ...
      if (editingEvent.subEventsDetail && editingEvent.subEventsDetail.length > 0) {
        ...
        setSubEvents(editingEvent.subEventsDetail);
  ...
  const updateSubEvent = (subEventId: string, field: keyof SubEvent, value: any) => {
    ...
    if (value && value.toString().trim()) {
      ...
      const fieldErrorMap: {[key: string]: string} = {
        ...
        timeAmPm: 'time',
        entryFees: 'entryFees',
        registrationDeadlineDate: 'regDeadline',
        registrationDeadlineHour: 'regDeadline',
        registrationDeadlineMinute: 'regDeadline',
        registrationDeadlineAmPm: 'regDeadline',
        otse: 'otse',
        performanceDurationMin: 'duration',
        performanceDurationMax: 'duration',
        timeLimitNotApplicable: 'duration'
      };

      const errorKey = fieldErrorMap[field];
      if (errorKey) {
        clearSubEventError(subEventId, errorKey);
      }
    }

    // Real-time validation for registration deadline vs event dates
    if (field === 'registrationDeadlineDate' || field === 'registrationDeadlineHour' || 
        field === 'registrationDeadlineMinute' || field === 'registrationDeadlineAmPm' ||
        field === 'startDate' || field === 'endDate') {
      
      setTimeout(() => {
        // Get the updated subEvent after state change
        setSubEvents(currentSubEvents => {
          const updatedSubEvent = currentSubEvents.find(se => se.id === subEventId);
          if (updatedSubEvent && updatedSubEvent.registrationDeadlineDate.trim() && updatedSubEvent.startDate.trim()) {
            const regDeadlineDate = new Date(updatedSubEvent.registrationDeadlineDate);
            const eventEndDate = updatedSubEvent.isSingleDate ? new Date(updatedSubEvent.startDate) : new Date(updatedSubEvent.endDate || updatedSubEvent.startDate);
            
            // Set time for proper comparison
            regDeadlineDate.setHours(
              parseInt(updatedSubEvent.registrationDeadlineHour) + (updatedSubEvent.registrationDeadlineAmPm === 'pm' && updatedSubEvent.registrationDeadlineHour !== '12' ? 12 : 0),
              parseInt(updatedSubEvent.registrationDeadlineMinute),
              0,
              0
            );
  ...
  const toggleSubEventActive = (subEventId: string) => {
    ...
    // If trying to activate the sub event, validate first
    if (!subEvent.isActive) {
      // Check if all organizing committee members have filled positions
      const missingPositions = organizingCommittee.filter(member => !member.designation?.trim());
      if (missingPositions.length > 0) {
        alert('Please fill in the Department field for all organizing committee members before activating.');
        return;
      }

      // Special validation for Sub Event-1 (first sub event) - check main form fields too
      ...
      // Validate sub event fields for all sub events
      const subEventValidation = validateSubEventFields(subEventId);
      if (!subEventValidation.isValid) {
        const subEventNumber = subEvents.findIndex(se => se.id === subEventId) + 1;
        alert(`Please complete all mandatory fields for Sub Event-${subEventNumber} first:\n\n${subEventValidation.missingFields.join('\n')}`);
        return;
      } else {
        // Clear sub event errors when validation passes
        setSubEventErrors(prev => {
    ...
    setSubEvents(prev => prev.map(subEvent => 
      subEvent.id === subEventId 
        ? { ...subEvent, isActive: !subEvent.isActive }
        : subEvent
    ));
  };
  ...
  const handleActivateLiveFeed = (subEventId: string) => {
    ...
    if (subEvent.activateLiveFeed) {
      ...
      // Clean up any stored streams
      if (cameraStreams[subEventId]) {
      ...
      alert('Live feed deactivated successfully!');
    } else {
      ...
      if (isDevelopment) {
        // For development - just simulate activation without actual camera access
        updateSubEvent(subEventId, 'activateLiveFeed', true);
        alert(
          '🎬 Live Feed Ready!\n\n' +
          '✅ Live streaming mode activated\n' +
          '✅ Event is ready for broadcasting\n' +
          '✅ All controls are functional\n\n' +
          '📝 Development Note:\n' +
          'Camera integration works in production with HTTPS.\n' +
          'Current mode simulates live feed functionality for testing.'
        );
        return;
      }

      // For production - attempt real camera access
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        // Browser doesn't support camera API - use fallback
        updateSubEvent(subEventId, 'activateLiveFeed', true);
        alert(
          '📱 Basic Live Mode Activated!\n\n' +
          '✅ Live feed enabled with basic functionality\n' +
          '✅ Compatible with your current browser\n' +
          '✅ Ready for event management\n\n' +
          '💡 Tip: For advanced camera features, use Chrome or Firefox.'
        );
        return;
      }

      // Try camera access for production
      navigator.mediaDevices.getUserMedia({ 
        video: { facingMode: 'user' }, 
        audio: false 
      })
      .then(stream => {
        // Success - store stream and activate
        setCameraStreams(prev => ({
          ...prev,
          [subEventId]: stream
        }));
        updateSubEvent(subEventId, 'activateLiveFeed', true);
        alert(
          '🎥 Professional Live Feed Activated!\n\n' +
          '✅ Camera access granted\n' +
          '✅ High-quality video streaming ready\n' +
          '✅ Full live broadcasting capabilities\n\n' +
          'Your event is ready for professional live streaming!'
        );
      })
      .catch(() => {
        // Camera failed - offer user-friendly fallback
        const useBasicMode = window.confirm(
          '📷 Camera Access Unavailable\n\n' +
          'Would you like to continue with Basic Live Mode?\n\n' +
          '✅ Click "OK" = Basic Live Mode (recommended)\n' +
          '❌ Click "Cancel" = Try camera setup later\n\n' +
          'Basic Live Mode includes all features except video preview.'
        );
        ...
        if (useBasicMode) {
          ...
          alert(
            '📡 Basic Live Mode Activated!\n\n' +
            '✅ Live feed enabled for your event\n' +
            '✅ All management features available\n' +
            '✅ Broadcasting ready (audio + screen)\n\n' +
            'Perfect for events that don\'t require video streaming!'
          );
        } else {
          alert(
            '🔧 Camera Setup Tips:\n\n' +
            ...
            'You can activate live feed anytime when ready!'
          );
        }
      });
    }
  ...
  const handleSave = () => {
    ...
    // Check if at least one sub-event is completed (all mandatory fields filled)
    ...
    // Additional validation: Check registration deadlines for all sub-events
    ...
    const eventDetails = allSubEvents.map((subEvent, index) => {
      ...
      return {
        ...
        deadline: subEvent.registrationDeadlineDate ? 
          `${new Date(subEvent.registrationDeadlineDate).toLocaleDateString('en-US', { 
            ...
            month: 'short', 
            year: 'numeric' 
          })} at ${subEvent.registrationDeadlineHour}:${subEvent.registrationDeadlineMinute}${subEvent.registrationDeadlineAmPm} (or till slots are available)` : 
          'TBD',
    ...
    const newEvent = {
      ...
      deadline: activeSubEvents[0]?.registrationDeadlineDate ? 
        `${new Date(activeSubEvents[0].registrationDeadlineDate).toLocaleDateString('en-US', { 
          ...
          month: 'short', 
          year: 'numeric' 
        })} at ${activeSubEvents[0].registrationDeadlineHour}:${activeSubEvents[0].registrationDeadlineMinute}${activeSubEvents[0].registrationDeadlineAmPm} (or till slots are available)` : 
        'TBD',
  ...
  const renderEventDetails = (subEvent: SubEvent, subEventIndex: number) => {
    ...
    return (
      <div className="space-y-0.5">
        ...
        <div className="grid grid-cols-[85px_1fr] items-center mt-0.5">
          ...
          <div className="flex gap-1 items-center">
            {subEvent.groupSingle === 'Single' ? (
              <>
                <div className="h-5 px-1 bg-gray-100 border border-gray-300 rounded text-sm text-black leading-none w-10 flex items-center justify-center">
                  1
                </div>
              ...
              <>
                <select
                  ...
                  className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none w-10 scrollbar-hide", currentErrors.participants)}
                >
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="flex gap-1 items-center max-w-full">
            <input
              ...
              min={getCurrentDate()}
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-black w-28", currentErrors.eventDates)}
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="flex gap-1">
            <select
              ...
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none w-12", currentErrors.time)}
            >
            ...
            <select
              ...
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none w-12 scrollbar-hide", currentErrors.time)}
            >
            ...
            <select
              ...
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none", currentErrors.time)}
            >
        ...
        {/* Perf. Duration */}
        <div className="grid grid-cols-[85px_1fr] items-center">
          <label className="text-sm text-black">Perf. Duration:</label>
          <div className="flex gap-1 items-center">
            <select
              ...
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none w-10", currentErrors.duration)}
            ...
            <select
              ...
              className={getInputClassName("h-6 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-normal w-10 scrollbar-hide", currentErrors.duration)}
              disabled={subEvent.timeLimitNotApplicable}
            >
              {generateNumberOptions(0, 59).map(minute => (
            ...
            {subEvent.rules.map((rule, ruleIndex) => (
              <div key={ruleIndex} className="flex gap-1 items-center">
                ...
                {subEvent.rules.length > 1 && (
                  <button
                    onClick={() => removeRule(subEvent.id, ruleIndex)}
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="relative" ref={entryFeesRef}>
            <input
              ...
              className={getInputClassName("w-full h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none", currentErrors.entryFees)}
          ...
          <div className="flex gap-1 items-center justify-between">
            <div className="flex gap-1 items-center">
              <input
                ...
                className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-black w-28", currentErrors.regDeadline)}
                style={{ colorScheme: 'light' }}
              ...
              <select
                ...
                className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none w-8", currentErrors.regDeadline)}
              >
                {generateNumberOptions(1, 12).map(hour => (
                  <option key={hour} value={hour}>{hour.toString().padStart(2, '0')}</option>
                ))}
              ...
              <select
                ...
                className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none w-8 scrollbar-hide", currentErrors.regDeadline)}
              >
                {generateNumberOptions(0, 59).map(minute => (
                  <option key={minute} value={minute}>{minute.toString().padStart(2, '0')}</option>
                ))}
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="h-5 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-none flex items-center justify-end">
            <button
        ...
        <div className="flex items-center justify-between mt-1">
          <button
            onClick={() => showDeleteConfirmation(subEvent.id)}
  ...
  return (
    <div className="w-full min-h-screen bg-white p-4">
      <h1 className="text-lg font-bold text-black mb-3">Add Event</h1>
      ...
      <div className="space-y-0.5">
        ...
        <div className="grid grid-cols-[85px_1fr] items-center">
          ...
          <div className="flex gap-1 items-center">
            <input
              ...
              min={getCurrentDate()}
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-black flex-1", mainFormErrors.startDate)}
              style={{ colorScheme: 'light' }}
            ...
            <input
              ...
              className={getInputClassName("h-5 px-1 bg-transparent border border-gray-300 rounded text-black flex-1", mainFormErrors.endDate)}
              style={{ colorScheme: 'light' }}
          ...
          <div className="space-y-1">
            {organizingCommittee.map((member, index) => (
              <div key={index} className="space-y-0.5">
                <div className="grid grid-cols-[80px_1fr_20px] items-center gap-1">
                  <label className="text-sm text-black">Name:</label>
                  <input
                    type="text"
                    value={member.isFirstMember ? 'John Doe' : (member.name || '')}
                ...
                <div className="grid grid-cols-[80px_1fr] items-center gap-1">
                  ...
                  <select
                    ...
                    className="h-6 px-1 bg-transparent border border-gray-300 rounded text-sm text-black leading-tight"
                    required
                  >
                    <option value="" disabled hidden></option>
                ...
                <div className="grid grid-cols-[80px_1fr] items-center gap-1">
                  ...
                  <input
                    type="tel"
                    value={member.isFirstMember ? '+91 9876543210' : (member.phone || '')}
                ...
                <div className="grid grid-cols-[80px_1fr] items-center gap-1">
                  ...
                  <input
                    type="email"
                    value={member.isFirstMember ? 'john.doe@college.edu' : (member.email || '')}
                    onChange={(e) => updateCommitteeMember(index, 'email', e.target.value)}
                    className="h-5 px-1 bg-gray-50 border border-gray-300 rounded text-sm text-black leading-none"
                    placeholder={member.isFirstMember ? "" : "Enter email"}
                    disabled={member.isFirstMember}
                  />
                </div>
        ...
        <div className="mt-3">
          <button
            onClick={handleSave}
            className="w-full h-8 bg-blue-600 text-white rounded text-sm hover:bg-blue-700 transition-colors flex items-center justify-center"
          >
            Save
          </button>
        </div>
